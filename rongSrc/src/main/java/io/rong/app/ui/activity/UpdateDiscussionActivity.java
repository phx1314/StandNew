...
public class UpdateDiscussionActivity extends BaseActionBarActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (getIntent() != null && getIntent().hasExtra("DEMO_DISCUSSIONIDS")
				&& getIntent().hasExtra("DEMO_DISCUSSIONNAME")) {
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		if (item.getItemId() == R.id.icon) {
			if (RongIM.getInstance() != null
				...
				RongIM.getInstance()
						.getRongIMClient()
						.setDiscussionName(mTargetId,
								mNewName.getText().toString(),
								new RongIMClient.OperationCallback() {
									@Override
									public void onSuccess() {
										WinToast.toast(
												UpdateDiscussionActivity.this,
												"讨论组名称修改成功");
										Intent intent = new Intent();
										intent.putExtra(
												"UPDATA_DISCUSSION_RESULT",
												mNewName.getText().toString());
										setResult(
												Constants.FIX_DISCUSSION_NAME,
												intent);
										finish();
									}
