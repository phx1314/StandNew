...
/**
 * Created by Bob on 15/11/3. 会话页面 1，设置 ActionBar title 2，加载会话页面 3，push 和 通知 判断
 */
public class ConversationActivity extends BaseApiActivity implements
	...
	private void checkTextInputEditTextChanged() {
		...
		InputProvider.MainInputProvider provider = RongContext.getInstance()
				.getPrimaryInputProvider();
			...
			textInputProvider.setEditTextChangedListener(new TextWatcher() {
				...
				public void onTextChanged(CharSequence s, int start,
					...
					if (mConversationType
						...
						if (s.length() > 0) {
							String str = s.toString().substring(
									s.toString().length() - 1,
									s.toString().length());
							...
							if (str.equals("@")) {
								...
								if (mTargetIds != null) {
									UriFragment fragment = (UriFragment) getSupportFragmentManager()
											.getFragments().get(0);
									// 得到讨论组的 targetId
									mTargetId = fragment.getUri()
											.getQueryParameter("targetId");
								}
	...
	private void reconnect(String token) {
		...
		RongIM.connect(token, new RongIMClient.ConnectCallback() {
			...
			public void onSuccess(String s) {
				...
				if (DemoContext.getInstance() != null)
					mGetMyGroupsRequest = DemoContext.getInstance()
							.getDemoApi()
							.getMyGroups(ConversationActivity.this);
			}
	...
	private void setGroupActionBar(String targetId) {
		...
		if (DemoContext.getInstance() != null) {
			getSupportActionBar().setTitle(
					DemoContext.getInstance().getGroupNameById(targetId));
		}
	}
	...
	private void setDiscussionName(String targetIds) {
		...
		if (DemoContext.getInstance() != null) {
			...
			for (int i = 0; i < ids.length; i++) {
				sb.append(DemoContext.getInstance().getUserInfoById(ids[i])
						.getName().toString());
				sb.append(",");
			}
	...
	private void setPrivateActionBar(String targetId) {
		...
		if (DemoContext.getInstance() != null) {
			...
			UserInfos userInfos = DemoContext.getInstance().getUserInfosById(
					targetId);
	...
	private void enterSettingActivity() {
		...
		if (mConversationType == Conversation.ConversationType.PUBLIC_SERVICE
			...
			Uri uri = Uri.parse("demo://" + getApplicationInfo().packageName)
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == 29 && resultCode == Constants.MESSAGE_REPLY) {
			if (data != null && data.hasExtra("REPLY_NAME")
					&& data.hasExtra("REPLY_ID")) {
			...
			if (groups.getCode() == 200) {
				...
				if (groups.getResult() != null) {
					...
					intent.setClass(ConversationActivity.this,
							MainActivity.class);
					intent.putExtra("PUSH_CONVERSATIONTYPE",
							mConversationType.toString());
	...
	private void setRealTime() {
		...
		mRealTimeBar.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (currentLocationStatus == null)
					currentLocationStatus = RongIMClient.getInstance()
							.getRealTimeLocationCurrentState(mConversationType,
									mTargetId);
		...
		if (!TextUtils.isEmpty(mTargetId) && mConversationType != null) {
			...
			RealTimeLocationConstant.RealTimeLocationErrorCode errorCode = RongIMClient
					.getInstance().getRealTimeLocation(mConversationType,
							mTargetId);
			if (errorCode == RealTimeLocationConstant.RealTimeLocationErrorCode.RC_REAL_TIME_LOCATION_SUCCESS
					|| errorCode == RealTimeLocationConstant.RealTimeLocationErrorCode.RC_REAL_TIME_LOCATION_IS_ON_GOING) {
				RongIMClient.getInstance().addRealTimeLocationListener(
						mConversationType, mTargetId, this);// 设置监听
				currentLocationStatus = RongIMClient.getInstance()
						.getRealTimeLocationCurrentState(mConversationType,
								mTargetId);
	...
	private void startRealTimeLocation() {
		RongIMClient.getInstance().startRealTimeLocation(mConversationType,
				mTargetId);
		Intent intent = new Intent(ConversationActivity.this,
				RealTimeLocationActivity.class);
	...
	private void joinRealTimeLocation() {
		RongIMClient.getInstance().joinRealTimeLocation(mConversationType,
				mTargetId);
		Intent intent = new Intent(ConversationActivity.this,
				RealTimeLocationActivity.class);
		intent.putExtra("conversationType", mConversationType.getValue());
	...
	private boolean closeRealTimeLocation() {
		...
		if (mConversationType != null && !TextUtils.isEmpty(mTargetId)) {
			...
			if (status == RealTimeLocationConstant.RealTimeLocationStatus.RC_REAL_TIME_LOCATION_STATUS_IDLE
					|| status == RealTimeLocationConstant.RealTimeLocationStatus.RC_REAL_TIME_LOCATION_STATUS_INCOMING) {
				return false;
			}
		}

		final AlterDialogFragment alterDialogFragment = AlterDialogFragment
				.newInstance("提示", "退出当前页面将会终止实时位置共享,确定退出？", "否", "是");
		alterDialogFragment
				.setOnAlterDialogBtnListener(new AlterDialogFragment.AlterDialogBtnListener() {
					@Override
	...
	private void showRealTimeLocationBar(
			RealTimeLocationConstant.RealTimeLocationStatus status) {

		if (status == null)
			status = RongIMClient.getInstance()
					.getRealTimeLocationCurrentState(mConversationType,
							mTargetId);

		final List<String> userIds = RongIMClient.getInstance()
				.getRealTimeLocationParticipants(mConversationType, mTargetId);

		if (status != null
				&& status == RealTimeLocationConstant.RealTimeLocationStatus.RC_REAL_TIME_LOCATION_STATUS_INCOMING) {

			if (userIds != null && userIds.get(0) != null
					&& userIds.size() == 1) {

				DemoContext
						.getInstance()
						.getDemoApi()
						.getUserInfo(userIds.get(0),
								new DemoApi.GetUserInfoListener() {
									...
									public void onSuccess(
										...
										runOnUiThread(new Runnable() {
											@Override
											public void run() {
												TextView textView = (TextView) mRealTimeBar
				...
				if (userIds != null && userIds.size() > 0) {
					...
					if (mRealTimeBar != null
							&& mRealTimeBar.getVisibility() == View.VISIBLE) {
		...
		if (userIds != null && userIds.size() > 0) {
			...
			if (mRealTimeBar != null
					&& mRealTimeBar.getVisibility() == View.GONE) {
	...
	public void onEventMainThread(
			RongEvent.RealTimeLocationMySelfJoinEvent event) {

		onParticipantsJoin(RongIM.getInstance().getRongIMClient()
				.getCurrentUserId());
	}
	...
	public void onStatusChange(
			final RealTimeLocationConstant.RealTimeLocationStatus status) {
	...
	public void onReceiveLocation(double latitude, double longitude,
		...
		EventBus.getDefault().post(
				RongEvent.RealTimeLocationReceiveEvent.obtain(userId, latitude,
						longitude));
	}
	...
	public void onParticipantsJoin(String userId) {
		EventBus.getDefault().post(
				RongEvent.RealTimeLocationJoinEvent.obtain(userId));
	...
	public void onParticipantsQuit(String userId) {
		EventBus.getDefault().post(
				RongEvent.RealTimeLocationQuitEvent.obtain(userId));
	}
