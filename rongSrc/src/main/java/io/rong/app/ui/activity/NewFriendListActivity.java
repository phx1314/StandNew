 ...
 */
public class NewFriendListActivity extends BaseApiActivity implements
		Handler.Callback {

	private static final String TAG = NewFriendListActivity.class
			.getSimpleName();
	...
	protected void initView() {
		...
		if (DemoContext.getInstance() != null) {
			getFriendHttpRequest = DemoContext.getInstance().getDemoApi()
					.getNewFriendlist(this);
			...
			if (obj instanceof Friends) {
				...
				if (friends.getCode() == 200) {
					if (friends.getResult().size() != 0) {
						...
						adapter = new NewFriendListAdapter(mResultList,
								NewFriendListActivity.this);
			...
			case 3:// 请求被添加
				...
				if (DemoContext.getInstance() != null)
					runOnUiThread(new Runnable() {
						@Override
						public void run() {
							mRequestFriendHttpRequest = DemoContext
									.getInstance()
									.getDemoApi()
									.processRequestFriend(
											mResultList.get(position).getId(),
											"1", new ApiCallback<Status>() {
												@Override
												public void onComplete(
														AbstractHttpRequest<Status> statusAbstractHttpRequest,
														Status status) {
													Log.e(TAG,
															"----mRequestFriendHttpRequest----onComplete---");

													ApiResult apiResult = mResultList
															.get(position);
													...
													mHandler.post(new Runnable() {
														...
														public void run() {
															UserInfo info = new UserInfo(
																	...
																	mResultList
																			.get(position)
																			.getPortrait() == null ? null
																			: Uri.parse(mResultList
																					.get(position)
																					.getPortrait()));
															if (DemoContext
																	.getInstance() != null) {
																if (DemoContext
																	...
																	DemoContext
																			.getInstance()
																			.insertOrReplaceUserInfo(
																					info,
																					"1");
																}
															}
															sendMessage(mResultList
																	.get(position)
																	.getId());
														}
													});
												...
												public void onFailure(
														AbstractHttpRequest<Status> statusAbstractHttpRequest,
														BaseException e) {
													if (mRequestFriendHttpRequest != null
															&& mRequestFriendHttpRequest
																	.equals(statusAbstractHttpRequest)) {
														Log.e(TAG,
																"----mRequestFriendHttpRequest----onFailure---"
																		+ e);
														// if (mDialog != null)
														// mDialog.dismiss();
													}
	...
	private void sendMessage(String id) {
		final AgreedFriendRequestMessage message = new AgreedFriendRequestMessage(
				id, "agree");
		if (DemoContext.getInstance() != null) {
			// 获取当前用户的 userid
			String userid = DemoContext.getInstance().getSharedPreferences()
					.getString(Constants.APP_USER_ID, Constants.DEFAULT);
			String username = DemoContext.getInstance().getSharedPreferences()
					.getString(Constants.APP_USER_NAME, Constants.DEFAULT);
			String userportrait = DemoContext.getInstance()
					.getSharedPreferences()
					.getString(Constants.APP_USER_PORTRAIT, Constants.DEFAULT);

			UserInfo userInfo = new UserInfo(userid, username,
					Uri.parse(userportrait));
			...
			if (RongIM.getInstance() != null
				...
				RongIM.getInstance()
						.getRongIMClient()
						.sendMessage(Conversation.ConversationType.PRIVATE, id,
								message, null, null,
								new RongIMClient.SendMessageCallback() {
									...
									public void onError(Integer messageId,
										...
										Log.e(TAG,
												Constants.DEBUG
														+ "------DeAgreedFriendRequestMessage----onError--");
									}
									...
									public void onSuccess(Integer integer) {
										Log.e(TAG,
												Constants.DEBUG
														+ "------DeAgreedFriendRequestMessage----onSuccess--"
														+ message.getMessage());
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		if (item.getItemId() == R.id.icon) {
			Intent intent = new Intent(NewFriendListActivity.this,
