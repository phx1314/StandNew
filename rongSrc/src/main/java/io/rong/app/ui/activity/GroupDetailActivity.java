 ...
 */
public class GroupDetailActivity extends BaseApiActivity implements
	...
	protected void initData() {
		...
		if (DemoContext.getInstance() != null) {
			HashMap<String, Group> groupHashMap = DemoContext.getInstance()
					.getGroupMap();

			if (getIntent().hasExtra("INTENT_GROUP")) {
				mApiResult = (ApiResult) getIntent().getSerializableExtra(
						"INTENT_GROUP");
			}
			if (mApiResult != null) {
				...
				if (groupHashMap != null
						&& groupHashMap.containsKey(mApiResult.getId()
								.toString())) {
					mess.what = HAS_JOIN;
				} else {
					mess.what = NO_JOIN;
	...
	public void onClick(View v) {
		if (v.getId() == R.id.join_group) {
			if (DemoContext.getInstance() != null) {

				if (mApiResult.getNumber().equals("500")) {
					WinToast.toast(GroupDetailActivity.this,
							R.string.group_is_full);
					return;
				}
				...
				mJoinRequest = DemoContext.getInstance().getDemoApi()
						.joinGroup(mApiResult.getId(), this);

			}
		} else if (v.getId() == R.id.quit_group) {
			if (DemoContext.getInstance() != null) {
				if (mDialog != null && !mDialog.isShowing())
					mDialog.show();
				mQuitRequest = DemoContext.getInstance().getDemoApi()
						.quitGroup(mApiResult.getId(), this);
			}

		} else if (v.getId() == R.id.chat_group) {
			if (RongIM.getInstance() != null)
				RongIM.getInstance()
						.getRongIMClient()
						.joinGroup(mApiResult.getId(), mApiResult.getName(),
		...
		if (mJoinRequest == request) {
			if (obj instanceof Status) {
				...
				if (status.getCode() == 200 && mApiResult != null) {
					...
					intent.putExtra("result", DemoContext.getInstance()
							.getGroupMap());
			...
			if (obj instanceof Status) {
				...
				if (status.getCode() == 200) {
					...
					if (RongIM.getInstance() != null) {
						RongIM.getInstance()
								.getRongIMClient()
								.quitGroup(mApiResult.getId(),
										new RongIMClient.OperationCallback() {
											@Override
											public void onSuccess() {
												WinToast.toast(
														GroupDetailActivity.this,
														R.string.group_quit_success);
											}
