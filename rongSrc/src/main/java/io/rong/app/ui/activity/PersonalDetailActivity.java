...
public class PersonalDetailActivity extends BaseApiActivity {
		...
		mAddFriend.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View view) {
				...
				if (currentUserId != null) {
					...
					mUserHttpRequest = DemoContext
							.getInstance()
							.getDemoApi()
							.sendFriendInvite(currentUserId, "请添加我为好友 ",
									PersonalDetailActivity.this);
				}
			}
		});
	...
	protected void initData() {
		...
		if (getIntent().hasExtra("USER")) {
			...
			if (userInfo != null && friendList != null) {
				...
				mPersonalImg
						.setResource(new Resource(userInfo.getPortraitUri()));
		...
		if (currentUserId != null)
			getUserInfoByUserIdHttpRequest = DemoContext.getInstance()
					.getDemoApi().getUserInfoByUserId(currentUserId, this);
	}

	@Override
	public void onCallApiSuccess(AbstractHttpRequest request, Object obj) {
		if (mDeleteFriendRequest != null
				&& mDeleteFriendRequest.equals(request)) {
			if (mDialog != null)
				mDialog.dismiss();
			if (obj instanceof Status) {
				...
				if (status.getCode() == 200) {
					...
					if (DemoContext.getInstance() != null
						...
						this.setResult(Constants.DELETE_USERNAME_REQUESTCODE,
								intent);
		...
		} else if (getUserInfoByUserIdHttpRequest != null
				&& getUserInfoByUserIdHttpRequest.equals(request)) {
			if (obj instanceof User) {
				...
				if (user.getCode() == 200) {
					...
					RongIM.getInstance().refreshUserInfoCache(
							new UserInfo(user.getResult().getId(), user
									.getResult().getUsername(), Uri.parse(user
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		if (item.getItemId() == R.id.per_item1) {
			if (DemoContext.getInstance() != null
					&& RongIM.getInstance().getRongIMClient() != null
					&& currentUserId != null) {
				RongIM.getInstance()
						.getRongIMClient()
						.addToBlacklist(currentUserId,
		...
		} else if (item.getItemId() == R.id.per_item2) {
					...
					new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							...
							if (DemoContext.getInstance() != null
								...
								mDeleteFriendRequest = DemoContext
										.getInstance()
										.getDemoApi()
										.deletefriends(currentUserId,
												PersonalDetailActivity.this);
							}
						}
					});
