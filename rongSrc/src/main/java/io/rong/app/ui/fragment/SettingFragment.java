 ...
 */
public class SettingFragment extends DispatchResultFragment implements
	...
	public View onCreateView(LayoutInflater inflater,
		...
		View view = inflater.inflate(R.layout.de_ac_friend_setting, container,
				false);

		mDeleteBtn = (Button) view.findViewById(R.id.de_fr_delete);
		mChatRoomRel = (RelativeLayout) view
				.findViewById(R.id.de_set_chatroom_name);
		mChatRoomName = (TextView) view.findViewById(R.id.de_chatroom_name);

		mAddNumberFragment = getChildFragmentManager().findFragmentById(
				R.id.de_fr_add_friend);
		mToTopFragment = getChildFragmentManager().findFragmentById(
				R.id.de_fr_to_top);
		...
		if (targetId != null) {
			...
			RongContext.getInstance().setOnMemberSelectListener(
					new RongIM.OnSelectMemberListener() {
						@Override
						public void startSelectMember(Context context,
								Conversation.ConversationType conversationType,
								String id) {
							mConversationType = Conversation.ConversationType
									.valueOf(getActivity().getIntent()
											.getData().getLastPathSegment()
											.toUpperCase(Locale.getDefault()));
							...
							in.putExtra("DEMO_FRIEND_CONVERSATTIONTYPE",
	 ...
	 *            会话类型
	 */
	private void showViewByConversationType(
	...
	public void onClick(View view) {
		...
		} else if (view.getId() == R.id.de_set_chatroom_name) {
			Intent intent = new Intent(getActivity(),
					UpdateDiscussionActivity.class);
			intent.putExtra("DEMO_DISCUSSIONIDS", targetId);
			if (mDiscussionName != null)
				intent.putExtra("DEMO_DISCUSSIONNAME",
						mDiscussionName.toString());
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case Constants.FIX_DISCUSSION_NAME:
			if (data != null)
				mChatRoomName.setText(data
						.getStringExtra("UPDATA_DISCUSSION_RESULT"));
