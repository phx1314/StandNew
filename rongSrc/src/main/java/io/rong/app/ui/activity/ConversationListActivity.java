...
/**
 * Created by Bob on 15/11/3. 会话列表，需要做 2 件事 1，push 重连，收到 push 消息的时候，做一下 connect
 * 操作 2，获取所加入的群组，并做 syncGroup 操作。demo 逻辑，需要同步当前所加入的群组
 */
public class ConversationListActivity extends BaseApiActivity {
	 ...
	 * <p/>
	 * 作用：由于在 manifest 中 intent-filter 是配置在 ConversationListActivity
	 * 下面，所以收到消息后点击notifacition 会跳转到 DemoActivity。 以跳到 MainActivity 为例： 在
	 * ConversationListActivity 收到消息后，选择进入 MainActivity，这样就把 MainActivity
			...
			public void onSuccess(String s) {
				...
				if (DemoContext.getInstance() != null)
					mGetMyGroupsRequest = DemoContext.getInstance()
							.getDemoApi()
							.getMyGroups(ConversationListActivity.this);
			}
