 ...
 */
public class DisturbActivity extends BaseActionBarActivity implements
		...
		public void handleMessage(Message msg) {
			...
			case 1:
				...
				if (msg != null) {
					...
					String time = DateUtils.dateToString(DateUtils.addMinutes(
							DateUtils.stringToDate(mStartTime, mTimeFormat),
							spanMins), mTimeFormat);
					mStartTimeNofication.setText(mStartTime);
					mEndTimeNofication.setText(time);

					editor = DemoContext.getInstance().getSharedPreferences()
							.edit();
					editor.putString("START_TIME", mStartTime);
					editor.putString("END_TIME", DateUtils.dateToString(
							DateUtils.addMinutes(DateUtils.stringToDate(
									mStartTime, mTimeFormat), spanMins),
							mTimeFormat));
					editor.commit();
				}
				break;
			...
			case 3:
				...
				if (DemoContext.getInstance().getSharedPreferences() != null) {
					String endtime = DemoContext.getInstance()
							.getSharedPreferences().getString("END_TIME", null);
					String starttimes = DemoContext.getInstance()
							.getSharedPreferences()
							.getString("START_TIME", null);

					if (endtime != null && starttimes != null
							&& !"".equals(endtime) && !"".equals(starttimes)) {
						Date datastart = DateUtils.stringToDate(starttimes,
								mTimeFormat);
						Date dataend = DateUtils.stringToDate(endtime,
								mTimeFormat);
						long spansTime = DateUtils.compareMin(datastart,
								dataend);
						...
						editor = DemoContext.getInstance()
	...
	protected void initData() {
		...
		if (DemoContext.getInstance().getSharedPreferences() != null) {
			mIsSetting = DemoContext.getInstance().getSharedPreferences()
					.getBoolean("IS_SETTING", false);
			if (mIsSetting) {
									...
									.getNotificationQuietHours(
											new RongIMClient.GetNotificationQuietHoursCallback() {
												...
												public void onSuccess(
														...
														int spanMins) {
													Log.e(TAG,
															"----yb----获取会话通知周期-onSuccess起始时间startTime:"
																	+ startTime
																	+ ",间隔分钟数spanMins:"
																	+ spanMins);
													if (spanMins > 0) {
	...
	public void onClick(View v) {
		...
		if (v.getId() == R.id.start_notification) {
			if (DemoContext.getInstance().getSharedPreferences() != null) {
				String starttime = DemoContext.getInstance()
						.getSharedPreferences().getString("START_TIME", null);
				if (starttime != null && !"".equals(starttime)) {
					hourOfDays = Integer.parseInt(starttime.substring(0, 2));
					minutes = Integer.parseInt(starttime.substring(3, 5));
				}
			}
			timePickerDialog = new TimePickerDialog(DisturbActivity.this,
					new TimePickerDialog.OnTimeSetListener() {
						...
						public void onTimeSet(TimePicker view, int hourOfDay,
							...
							mStartTimeNofication.setText(mStartTime);
							SharedPreferences.Editor editor = DemoContext
							...
							if (DemoContext.getInstance()
								...
								String endtime = DemoContext.getInstance()
										.getSharedPreferences()
										.getString("END_TIME", null);
								if (endtime != null && !"".equals(endtime)) {
									Date datastart = DateUtils.stringToDate(
											mStartTime, mTimeFormat);
									Date dataend = DateUtils.stringToDate(
											endtime, mTimeFormat);
									long spansTime = DateUtils.compareMin(
											datastart, dataend);
									setConversationTime(mStartTime,
											(int) Math.abs(spansTime));
								}
							}
						}
		...
		} else if (v.getId() == R.id.end_notification) {
			if (DemoContext.getInstance().getSharedPreferences() != null) {
				String endtime = DemoContext.getInstance()
						.getSharedPreferences().getString("END_TIME", null);
				if (endtime != null && !"".equals(endtime)) {
					...
					new TimePickerDialog.OnTimeSetListener() {
						...
						public void onTimeSet(TimePicker view, int hourOfDay,
							...
							if (DemoContext.getInstance()
								...
								if (starttime != null && !"".equals(starttime)) {
									Date datastart = DateUtils.stringToDate(
											starttime, mTimeFormat);
									Date dataend = DateUtils.stringToDate(
											mEndTime, mTimeFormat);
									long spansTime = DateUtils.compareMin(
											datastart, dataend);
									Log.e("", "------结束时间----" + mEndTime);
			...
			timePickerDialog.show();
		} else if (v.getId() == R.id.notification_checkbox) {
	...
	private void setConversationTime(final String startTime, final int spanMins) {
		...
		if (RongIM.getInstance() != null && startTime != null
				&& !"".equals(startTime)) {
			mHandler.post(new Runnable() {
				@Override
				public void run() {
					if (spanMins > 0 && spanMins < 1440) {
						Log.e("", "----设置勿扰时间startTime；" + startTime
								+ "---spanMins:" + spanMins);

						RongIM.getInstance()
								.getRongIMClient()
								.setNotificationQuietHours(startTime, spanMins,
										new RongIMClient.OperationCallback() {

											@Override
											public void onSuccess() {
												Log.e(TAG,
														"----yb----设置会话通知周期-onSuccess");
												SharedPreferences.Editor editor = DemoContext
														.getInstance()
														.getSharedPreferences()
														.edit();
												editor.putBoolean("IS_SETTING",
														true);
												editor.apply();
												mHandler.post(new Runnable() {
													@Override
